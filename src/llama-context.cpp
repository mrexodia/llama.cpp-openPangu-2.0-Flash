...
uint32_t llama_context::graph_max_nodes(uint32_t n_tokens) const {
    if (model.arch == LLM_ARCH_QWEN3NEXT ||
        ...
        model.arch == LLM_ARCH_OPENPANGU_V2) {
        // mHC (per-layer sinkhorn over many iterations) creates a very large graph
        return std::max<uint32_t>(n_tokens * 64, 160u * model.n_tensors());
    }
